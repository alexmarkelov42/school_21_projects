...
void MainWindow::on_pushButton_screenshot_clicked() {  // скриншот
  //  if (!QDir("/../../3dviewer_screenshots").exists()) {
  //    QDir().mkdir("/../../3dviewer_screenshots");
  //  }
...
void MainWindow::on_pushButton_screencast_clicked() {
  // start_second_Thread();
  // std::thread th(&MainWindow::create_gif, this);
  // th.detach();
  create_gif();
}

// void MainWindow::start_second_Thread() {
//   thread = new QThread(this);
//   QObject::connect(thread, &QThread::started, this, &MainWindow::create_gif);
//   //  QObject::connect(thread, &QThread::finished, this,
//   &QThread::deleteLater); thread->start();
// }
...
void MainWindow::create_gif() {
  ...
  QPixmap screenshot;
  QImage image;
  // int devicePixelRatio = ui->openGLWidget->devicePixelRatio();  // считывание
  // плотности пикселей
  ...
  //  this->filePath = QFileDialog::getOpenFileName(
  //      this, "Choose file", "../../", "All Files (*.*);; OBJ Files
  //      (*.obj);");
  ...
  //          num_of_vertexes * 3);
  //  scaleX(&prev_scale, &sscale, vertexes, num_of_vertexes * 3);
  //  sscale = ui->doubleSpinBox_scale->value();

  for (int i = 0; i < 50; i += 1) {
    screenshot = ui->openGLWidget->grab();  // скриншот
    // screenshot.setDevicePixelRatio(devicePixelRatio);  // подгон плотности
    // пикселей
