...
BEGIN
    RETURN QUERY
        WITH data AS (SELECT DISTINCT peer_nickname as peername
                      FROM checks
                               JOIN xp x on checks.id = x.check_id
                      WHERE task_name = task1
                      INTERSECT
                      SELECT DISTINCT peer_nickname as peername
                      FROM checks
                               JOIN xp x on checks.id = x.check_id
                      WHERE task_name = task2
                      EXCEPT
                      SELECT DISTINCT peer_nickname as peername
                      FROM checks
                               JOIN xp x on checks.id = x.check_id
                      WHERE task_name = task3)
        SELECT peername
        FROM data;
END
$$;

--SELECT * FROM fnc_peers_finished_1_and_2_but_not_3('C3_s21_bash_utils', 'C4_s21_math', 'CPP1_s21_matrixplus');
